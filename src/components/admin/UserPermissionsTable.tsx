...
import ApprovalLimitsDialog from "./ApprovalLimitsDialog";
...
const UserPermissionsTable = ({ users, setUsers, onDeleteUser }: UserPermissionsTableProps) => {
  ...
  const updatePermission = (userId: string, permissionKey: keyof User["permissions"], value: boolean) => {
  ...
  // Update user approval limits
  const updateApprovalLimits = (userId: string, limits: ApprovalLimits) => {
    const updatedUsers = users.map(user => {
      if (user.id === userId) {
        return {
          ...user,
    ...
    toast({
      title: "Approval limits updated",
      description: `User approval limits have been successfully updated.`,
    });
  };

  // Open limits dialog for a user
  const openLimitsDialog = (user: User) => {
  ...
  return (
    <>
                            ...
                            <>
                              ...
                              {user.approvalLimits && (
                                <span className="text-xs text-muted-foreground mt-1">
                                  Max: {formatCurrency(Math.max(
                                    user.approvalLimits.eft || 0,
                                    user.approvalLimits.wire || 0, 
                                    user.approvalLimits.transfer || 0,
                                    user.approvalLimits.emailTransfer || 0
                                  ))}
                                </span>
                              )}
                            </>
      ...
      {/* Approval Limits Dialog */}
      {selectedUser && (
        <ApprovalLimitsDialog
          user={selectedUser}
          open={limitsDialogOpen}
          onOpenChange={setLimitsDialogOpen}
          onSave={updateApprovalLimits}
        />
      )}
    </>
  );

...
const AIAgentInterface = ({ onExecuteTransaction, accounts }: AIAgentProps) => {
  ...
  const processCommand = async (command: string) => {
    ...
    // In a real-world app, you might want to use a proper NLP service
    try {
      ...
      if (lowerCommand.includes("transfer")) type = "transfer";
        ...
        addMessage("I'm not sure what kind of transaction you want to make. Please specify if you want to do a transfer, wire, EFT, bill payment, email transfer, or foreign exchange.", false);
  ...
  return (
    <Drawer>
      <DrawerTrigger asChild>
        <Button 
          variant="outline" 
          className="fixed bottom-4 right-4 h-12 w-12 rounded-full shadow-lg p-0 bg-primary hover:bg-primary/90"
        >
        ...
        <Card className="border-0 rounded-none h-full flex flex-col">
          ...
          <CardFooter className="border-t p-4 absolute bottom-0 left-0 right-0 bg-background">
            <form onSubmit={handleSubmit} className="flex w-full gap-2">
              <Input
                placeholder={isListening ? "Listening..." : "Ask me to make a transaction..."}
                value={input}

      ...
      <CardContent>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div className="rounded-lg bg-green-50 p-4">
            <div className="font-medium text-green-700 mb-2">Upcoming Shopify Payouts</div>
            <ul className="text-sm space-y-1">
              {data.shopifyPayouts
                .filter((p: any) => p.expectedDate || new Date(p.date) > new Date())
                .map((p: any, i: number) => (
                <li key={i}>
                  {p.expectedDate && <span className="text-green-600 font-medium">Expected: {p.expectedDate}</span>}
                  {p.date && new Date(p.date) > new Date() && <span className="text-green-800 font-medium">{p.date}</span>}
          ...
          <div className="rounded-lg bg-purple-50 p-4">
            <div className="font-medium text-purple-700 mb-2">Upcoming Amazon Payouts</div>
            <ul className="text-sm space-y-1">
              {data.amazonPayouts
                .filter((p: any) => p.expectedDate || new Date(p.date) > new Date())
                .map((p: any, i: number) => (
                <li key={i}>
                  ...
                  {p.date && new Date(p.date) > new Date() && <span className="text-purple-800 font-medium">{p.date}</span>}
